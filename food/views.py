...
# Imports from my app
from .models import Food, Category, MySelection, NutritionGrade
...
def account(request):
    ...
    return render(request, 'food/account/account.html')
...
####### HEALTHY FOODS SELECTED BY THE USER #######
...
####### FOODS SEARCH BY CATEGORY - IF LOGGED IN #######
@login_required(login_url='/account/signin/', redirect_field_name='redirection_vers')
def display_per_category(request):
    """View to the page that displays the substitute food products related to the category
    selected by the user. Only the foods with A or B as a nutriscore are displayed.
    Possibility to save an healthy product."""
    posted = False
    paginate = False
    substitutes = {}
    ...
    if request.method == 'GET':
        ...
        ##########    DISPLAY SUBSTITUTE FOODS   ##########
        # Match the option value of the select form field with the real name of the category.
        match = {
            'biscuits': 'Biscuits',
            'charcuteries': 'Charcuteries',
            'chips_frites': 'Chips et frites',
            'confitures': 'Confitures',
            'crepes_galettes': 'Crêpes et galettes',
            'desserts': 'Desserts',
            'desserts_chocolat': 'Desserts au chocolat',
            'fruits_coques': 'Fruits à coques',
            'fruits_secs': 'Fruits secs',
            'gateaux': 'Gâteaux',
            'pains': 'Pains',
            'pates_tartiner': 'Pâtes à tartiner',
            'petit_dejeuners': 'Petit-déjeuners',
            'plats_viande': 'Plats à base de viande',
        ...
        for key, value in match.items():
            if key == category:
                category = value
        # Then, for the selected category, make the substitutes list with foods of that category
        # and that only have A or B as a nutriscore (nutrition_grade).
        try:
            substitutes = Food.objects.filter(
                category=Category.objects.get(
                    name=category), nutrition_grade__lt=NutritionGrade.c)
            substitutes = substitutes.order_by('nutrition_grade', 'nutrition_score')
        except Food.DoesNotExist:
            return substitutes
    ...
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            if verify.exists():
                # Redirect to the same foodresult page (same number) and get the substitutes list.
                return redirect(
                    reverse('categories') + '?category=' + str(category) + '&page=' + str(page))
            # The case it's a new selected food by the user.
            else:
                # Get or create a profile (MySelection) with the connected user.
                saved, is_new_profile = MySelection.objects.get_or_create(user=request.user)
    ...
    return render(request, 'food/foodcategories.html', context)
